...
import { sha256, validateManifest } from "../domain/manifest.js";
...
const forbiddenPhp = [
  ...
  { label: "literal remote or protocol-relative URL", pattern: /(?:https?:)?\/\//i },
  { label: "data or javascript URL", pattern: /\b(?:data|javascript|vbscript|file):/i },
...
async function listPhp(root: string): Promise<string[]> {
  ...
  async function visit(directory: string): Promise<void> {
    for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
      ...
      else if (entry.isSymbolicLink()) throw new Error(`Symbolic links are prohibited in staging: ${full}`);
...
async function phpLint(filePath: string): Promise<void> {
  try {
    await runProcess("php", ["-l", filePath], { timeoutMs: 30_000, maxOutputBytes: 256_000 });
  } catch (error) {
    throw new Error(`PHP syntax failed for ${path.basename(filePath)}: ${error instanceof Error ? error.message : String(error)}`);
  }
}

export async function validateGeneratedHomepage(
  stagingRoot: string,
  themePath: string,
  manifestInput: HomepageManifest,
  secrets: string[]
): Promise<HomepageManifest> {
  ...
  const expectedPaths = [
    templatePath,
    ...manifest.parts.map((part) => path.join(stagingRoot, "template-parts", "homepage", part.filename))
  ];
  const actual = new Set(phpFiles.map((file) => path.resolve(file).toLowerCase()));
  for (const expected of expectedPaths) {
    assertPathInside(stagingRoot, expected);
    if (!actual.has(path.resolve(expected).toLowerCase())) {
      throw new Error(`Manifest file is missing: ${path.basename(expected)}`);
    }
  }
  const template = await fs.readFile(templatePath, "utf8");
  if (!/Template Name:\s*.+/i.test(template) || !/\bget_header\s*\(/.test(template) || !/\bget_footer\s*\(/.test(template)) {
    throw new Error("Page template is missing required WordPress template calls/header");
  }
  const calls = [...template.matchAll(/get_template_part\s*\(\s*['"]([^'"]+)['"]\s*\)/g)].map((match) => match[1]);
  const expectedCalls = manifest.parts.map((part) =>
    `template-parts/homepage/${part.filename.replace(/\.php$/, "")}`
  );
  if (calls.length !== 10 || calls.some((call, index) => call !== expectedCalls[index])) {
    throw new Error("Page template must call exactly the 10 manifest parts in order");
  }
  for (const file of expectedPaths) {
    const content = await fs.readFile(file, "utf8");
    if (!/defined\s*\(\s*['"]ABSPATH['"]\s*\)/.test(content)) {
      throw new Error(`Missing ABSPATH guard: ${path.basename(file)}`);
    }
    ...
    for (const secret of secrets.filter((value) => value.length >= 4)) {
      if (content.includes(secret)) throw new Error(`Secret detected in generated output: ${path.basename(file)}`);
    }
    await phpLint(file);
  }
  for (const file of expectedPaths) {
  ...
  for (let index = 0; index < manifest.parts.length; index += 1) {
    manifest.parts[index]!.checksum_sha256 = await sha256(expectedPaths[index + 1]!);
  }
