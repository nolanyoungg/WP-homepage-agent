...
4. Compile, lint, and validate the real configured model:
...
| `npm run validate` | Compile TypeScript and run ESLint. |
...
No fixture-based or simulated test suite is stored in this repository. Run
live validation on the configured operator environment:
...
A passing build or lint run does not prove a live model, LM Link, Messages,
Live Link, or WordPress integration.

## Repository policy

Do not commit trackers containing private business data, generated run state,
credentials, machine names, personal paths, IP addresses, model logs, or
screenshots with Live Link credentials. See the
[dependency-risk policy](docs/DEPENDENCY-RISK.md).
